const { webcrypto: { subtle }, ...crypto } = require('crypto')
...
async function encrypt (cleartext, key, iv) {
  ...
  return subtle.encrypt({ name: 'AES-CTR', counter: iv, length: 16 }, key, cleartext).then(Buffer.from)
}

async function decrypt (ciphertext, key, iv) {
  key = await subtle.importKey('raw', key, 'AES-CTR', false, ['decrypt'])
  return subtle.decrypt({ name: 'AES-CTR', counter: iv, length: 16 }, key, ciphertext).then(Buffer.from)
}
