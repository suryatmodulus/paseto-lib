...
const { V3: { encrypt, decrypt, generateKey }, errors } = require('../../lib')
...
test('not a v3.local paseto', async t => {
  const key = await generateKey('local')

  let paseto = await encrypt({}, key)
  paseto = paseto.replace('v3.local', 'v2.local')

  await t.throwsAsync(() => decrypt(paseto, key), {
    message: 'token is not a v3.local PASETO',
...
test('invalid key length', async t => {
  ...
  await t.throwsAsync(() => encrypt({}, key), {
    message: 'v3.local secret key must be 32 bytes long symmetric key',
    instanceOf: TypeError
  })
})
