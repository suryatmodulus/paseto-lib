...
test('decode must be a supported header', t => {
  ...
  t.throws(
    () => decode('v2.foo.'),
    { instanceOf: errors.PasetoNotSupported, code: 'ERR_PASETO_NOT_SUPPORTED', message: 'unsupported PASETO purpose' }
  )
})
...
test('skips parsing the payload for local tokens', t => {
  t.deepEqual(
    decode('v2.local.eyJpYXQiOiIyMDE5LTA3LTAyVDEyOjA4OjI1LjE5OVoifS3L7e7t5YUwI1uKlW15mNNCI-aJYDDOJyEEe567xxYTcMJ4vD4MJuJiTEd1buIMQ0JoBk6kZzyknKNZKa57dg8.Zm9v'),
    {
      footer: Buffer.from('foo'),
      payload: undefined,
      purpose: 'local',
