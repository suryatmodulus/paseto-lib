...
test('V3 generateKey handles invalid purposes', async t => {
  await t.throwsAsync(
    V3.generateKey('foo'),
    { instanceOf: errors.PasetoNotSupported, code: 'ERR_PASETO_NOT_SUPPORTED', message: 'unsupported v3 purpose' }
  )
})
...
test('V4 generateKey handles invalid purposes', async t => {
  await t.throwsAsync(
    V4.generateKey('foo'),
    { instanceOf: errors.PasetoNotSupported, code: 'ERR_PASETO_NOT_SUPPORTED', message: 'unsupported v4 purpose' }
  )
})
